...
const KNOCKOUT_ROUNDS = [
  ...
  { id: "third-place", label: "季军战", matchLabel: "季军战", pattern: /(Third Place|3rd-?Place)/i, slots: 1 }
];
...
function formatPlaceholderTeamName(name = "待定") {
  return String(name)
    ...
    .replace(/Semifinals? (\d+) Loser/i, "半决赛第$1场负者");
}
...
const DASHBOARD_DATA_URL = "./data/latest.json?v=20260715-scoreboard-limit";
const DASHBOARD_DATA_REFRESH_MS = 60 * 60 * 1000;
const DASHBOARD_DATE_GUARD_MS = 60 * 1000;
const DASHBOARD_TIME_ZONE = "Asia/Shanghai";

let dashboardData = null;
let liveScoreTimer = null;
let dashboardDataTimer = null;
let dashboardDateGuardTimer = null;
...
function formatPredictionItem(label, value, meta = "") {
  return `
...
function topMarketSelections(match, marketName) {
  const source = match.preview.bettingSources?.find((item) =>
    item.markets?.some((market) => market.name === marketName)
  );
  const market = source?.markets?.find((item) => item.name === marketName);
  return market?.selections ?? [];
}

function renderOutcomePrediction(match) {
  const outcomes = [...(match.preview.aiPrediction?.outcomes ?? match.preview.probabilities ?? [])].sort((a, b) => b.probability - a.probability);
  if (!outcomes.length) {
    return `<div class="prediction-empty">暂无足够胜平负概率。</div>`;
  }

  const first = outcomes[0];
  const second = outcomes[1] ?? outcomes[0];
  const upset = outcomes[outcomes.length - 1] ?? outcomes[0];

  return `
    <div class="prediction-group">
      <div class="prediction-group-title">胜平负</div>
      <div class="prediction-item-grid">
        ${formatPredictionItem("第一选项", first.label, first.reason ?? `${first.probability}%`)}
        ${formatPredictionItem("第二选项", second.label, second.reason ?? `${second.probability}%`)}
        ${formatPredictionItem("冷门选项", upset.label, upset.reason ?? `${upset.probability}%`)}
      </div>
    </div>
  `;
}

function scoreDirection(card) {
  if (card.direction) return card.direction;
  const [homeScore, awayScore] = String(card.score ?? "")
    .split("-")
...
function bestScoreForOutcome(cards, outcome) {
  return cards.find((card) => scoreDirection(card) === outcome?.key) ?? cards[0] ?? null;
}

function scorePredictionMeta(card, outcome) {
  const price = card?.rows?.[0]?.current ? `约 ${card.rows[0].current}` : "";
  const direction = outcome?.label ? `对应 ${outcome.label}` : "";
  return [direction, price].filter(Boolean).join(" · ");
}

function renderScorePrediction(match) {
  const aiScores = match.preview.aiPrediction?.scores ?? [];
  if (aiScores.length) {
    return `
      <div class="prediction-group">
        <div class="prediction-group-title">比分</div>
        <div class="prediction-item-grid">
          ${aiScores
            .map((item) => formatPredictionItem(item.label, item.score, item.reason))
            .join("")}
        </div>
      </div>
    `;
  ...
  const outcomes = [...(match.preview.probabilities ?? [])].sort((a, b) => b.probability - a.probability);
  const cards = match.preview.scoreOdds?.allCards ?? match.preview.scoreOdds?.cards ?? [];
  if (!cards.length) {
    return `<div class="prediction-empty">暂无足够比分分布。</div>`;
  }

  const firstOutcome = outcomes[0];
  const secondOutcome = outcomes[1] ?? outcomes[0];
  const upsetOutcome = outcomes[outcomes.length - 1] ?? outcomes[0];
  const first = bestScoreForOutcome(cards, firstOutcome);
  const second = bestScoreForOutcome(cards, secondOutcome);
  const upset = bestScoreForOutcome(cards, upsetOutcome);

  return `
    <div class="prediction-group">
      <div class="prediction-group-title">比分</div>
      <div class="prediction-item-grid">
        ${formatPredictionItem("第一选项", first.score, scorePredictionMeta(first, firstOutcome))}
        ${formatPredictionItem("第二选项", second.score, scorePredictionMeta(second, secondOutcome))}
        ${formatPredictionItem("冷门选项", upset.score, scorePredictionMeta(upset, upsetOutcome))}
      </div>
    </div>
  `;
}

function renderSideMarketPrediction(match, marketName, title) {
  const aiPrediction = marketName === "大小球" ? match.preview.aiPrediction?.total : match.preview.aiPrediction?.spread;
  if (aiPrediction?.pick) {
    return `
      <div class="prediction-side-card">
        <span>${title}</span>
        <strong>${aiPrediction.pick}</strong>
        <em>${aiPrediction.reason}</em>
      </div>
    `;
  }

  const selections = topMarketSelections(match, marketName);
  if (!selections.length) {
    return `
      <div class="prediction-side-card">
        <span>${title}</span>
        <strong>暂无稳定市场</strong>
      </div>
    `;
  }

  const pick = [...selections].sort((a, b) => decimalValue(a.current) - decimalValue(b.current))[0];
  const line = pick.line && pick.line !== "N/A" ? ` ${pick.line}` : "";
  const trend = pick.movement && pick.movement !== "-" ? `走势 ${pick.movement}` : "走势持平";

  return `
    <div class="prediction-side-card">
      <span>${title}</span>
      <strong>${pick.label}${line}</strong>
      <em>${pick.current} · ${trend}</em>
    </div>
  `;
}

function renderPredictionRationale(match) {
  const rationale = match.preview.aiPrediction?.rationale ?? [];
  const sources = match.preview.aiPrediction?.externalSources ?? [];
  if (!rationale.length && !sources.length) return "";

  return `
    <div class="prediction-rationale">
      <div class="prediction-group-title">分析依据</div>
      ${rationale.map((item) => `<p>${item}</p>`).join("")}
      ${
        sources.length
...
function renderAIAnalysisPrediction(match) {
  return `
    <section class="ai-prediction-panel">
      <div class="ai-prediction-head">
        <div>
          <span>AI分析预测</span>
          <strong>${match.homeTeam.name} vs ${match.awayTeam.name}</strong>
        </div>
        <em>倾向强度：${match.preview.heat ?? match.preview.confidence ?? "观察中"}</em>
      </div>
      ${renderOutcomePrediction(match)}
      ${renderScorePrediction(match)}
      <div class="prediction-side-grid">
        ${renderSideMarketPrediction(match, "大小球", "大小波")}
        ${renderSideMarketPrediction(match, "让分", "让分输赢")}
      </div>
      ${renderPredictionRationale(match)}
      <p class="prediction-disclaimer">备注：只提供分析比赛预测，不提供任何娱乐方面的建议。</p>
    </section>
  `;
}
  ...
  if (match.status === "pre") {
    return `
      <div class="preview-box">
        ${renderAIAnalysisPrediction(match)}
...
function formatLocalKickoff(isoString) {
...
function matchFromScoreboardEvent(event) {
  const competition = event.competitions?.[0];
  const competitors = competition?.competitors ?? [];
  const home = competitors.find((item) => item.homeAway === "home");
  const away = competitors.find((item) => item.homeAway === "away");
  const note = competition?.altGameNote ?? "";
  const group = groupNameFromNote(note);
  const status = competition?.status?.type?.state ?? event.status?.type?.state ?? "pre";
  const shootout = extractShootoutScoreFromEvent(competition, home, away);

  return {
    id: event.id,
    date: event.date,
    localKickoff: formatLocalKickoff(event.date),
    stage: note,
    group,
    round: group === "Knockout" ? knockoutRoundFromNote(note) : "",
    status,
    statusLabel: competition?.status?.type?.description ?? event.status?.type?.description ?? "Scheduled",
    shortDetail: competition?.status?.type?.shortDetail ?? event.status?.type?.shortDetail ?? "",
    shootout,
    ...
    analysisFactors: [],
    summaryLink: event.links?.find((link) => link.rel?.includes("summary"))?.href ?? ""
  };
}

function applyScoreboardFallback(scoreboard) {
  if (!dashboardData) return false;

  const todayKey = getDashboardDateKey();
  const tomorrowKey = getDashboardDateKey(new Date(Date.now() + 24 * 60 * 60 * 1000));
  const matches = (scoreboard.events ?? []).map(matchFromScoreboardEvent);
  const todayMatches = matches.filter((match) => getDashboardDateKey(new Date(match.date)) === todayKey);
  const tomorrowMatches = matches.filter((match) => getDashboardDateKey(new Date(match.date)) === tomorrowKey);
  const knockoutMatches = matches.filter((match) => match.group === "Knockout");

  if (!todayMatches.length && !tomorrowMatches.length) return false;

  dashboardData.meta.snapshotDate = todayKey;
  dashboardData.meta.generatedAt = new Date().toISOString();
  dashboardData.today.matches = todayMatches;
  dashboardData.today.date = todayKey;
  dashboardData.tomorrow.matches = tomorrowMatches;
  dashboardData.tomorrow.date = tomorrowKey;
  if (knockoutMatches.length) dashboardData.knockout.matches = knockoutMatches;
  refreshHeadlineCounts(dashboardData);
  dashboardData.headline.summary = `公开比分兜底已更新到 ${todayKey}，静态模型数据等待下一次 GitHub 小时任务同步。`;
...
async function refreshLiveScores() {
  ...
  try {
    ...
    if (dashboardData.meta?.snapshotDate !== getDashboardDateKey()) {
      applyScoreboardFallback(scoreboard);
      return;
    }
...
async function fetchDashboardData() {
  const response = await fetch(`${DASHBOARD_DATA_URL}&_=${Date.now()}`, { cache: "no-store" });
  if (!response.ok) throw new Error(`最新数据 ${response.status}`);
...
async function refreshDashboardData({ force = false } = {}) {
  try {
    ...
    const changed =
      force ||
      !dashboardData ||
      nextData.meta?.generatedAt !== dashboardData.meta?.generatedAt ||
      nextData.meta?.snapshotDate !== dashboardData.meta?.snapshotDate;
    if (!changed) return;

    renderDashboard(nextData);
    await refreshLiveScores();
  } catch (error) {
    console.warn("静态数据刷新失败", error);
  }
}
...
function refreshDashboardDataAfterDateChange() {
  if (!dashboardData?.meta?.snapshotDate) return;
  if (dashboardData.meta.snapshotDate !== getDashboardDateKey()) {
    refreshDashboardData({ force: true });
  }
}

function startDashboardDataRefresh() {
  if (dashboardDataTimer) clearInterval(dashboardDataTimer);
  if (dashboardDateGuardTimer) clearInterval(dashboardDateGuardTimer);
  dashboardDataTimer = setInterval(refreshDashboardData, DASHBOARD_DATA_REFRESH_MS);
  dashboardDateGuardTimer = setInterval(refreshDashboardDataAfterDateChange, DASHBOARD_DATE_GUARD_MS);

  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) {
      refreshDashboardData();
      refreshDashboardDataAfterDateChange();
    }
  });
